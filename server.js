...
const youtube = new YouTube("AIzaSyC-XCDTnFfgnFw7ztWPUJ1j4N2u_eFE9jY"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyC-XCDTnFfgnFw7ztWPUJ1j4N2u_eFE9jY"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@842053674292346910>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (message.content === "السلام عليكم") {
    message.channel.send("**:heart:وعليكم السلام ورحمة الله وبركاته نورت المتجر:heart:**");
...
client.on("message", message => {
  if (message.content === "اقوي متجر") {
    message.channel.send("**:heart:انت الاقوي يا عمري انت:heart:**");
    message.channel.sendFile("");
  }
});

client.on("message", message => {
  if (message.content === "افضل متجر") {
    message.channel.send("**:heart:انت الافضل يا عمري انت:heart:**");
    message.channel.sendFile("");
  }
});
////تعديل غير اساسي
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setby")) {
    ...
    client.on("guildMemberAdd", member => {
      ...
      var channel = member.guild.channels.find(gg => gg.name === "𝑳𝒐𝒈𝒔"); //// تعديل اساسي
      if (!channel) return;
...
client.on("guildMemberAdd", async member => {
  ...
  if (channel) {
    ...
    //Avatar
    var imageX = 242; //X coordinate
    var imageY = 164; //Y coordinate
    var imageRadius = 50; //نصف قطر الصورة الدائرية

    //Member Name
    var nameSize = "16pt"; //حجم خط الاسم
    var nameKind = "Zen Antique Soft "; //Zen Antique Soft    Source Sans Pro (OT1)نوع خط الاسم
    var nameColor = "#FFFFFF"; //لون خط الاسم

    //Name Position
    var nameX = 380; //position x
    var nameY = 158; //position y
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "تقديم")) {
    if (!message.channel.guild) return;
    if (message.author.bot) return;
    let channel = message.guild.channels.find(gg => gg.name === "تقديمات الادارة");
    if (!channel)
      return message.reply(
    ...
    if (channel) {
      message.channel.send(message.member + ", **:timer:**").then(m => {
        m.edit(message.member + ", **اسمك / عمرك / من وين**");
        m.channel
          ...
          .then(m1 => {
            ...
            m.edit(message.member + ", **:timer:**").then(m => {
              m.edit(message.member + ", **خبراتك بالبرمجة**");
              setTimeout(() => {
                m.delete();
              }, 10000);
              m.channel
                ...
                .then(m2 => {
                  ...
                  message.channel
                    ...
                    .then(m => {
                      m.edit(message.member + ", **خبراتك بالتصميم**");
                      setTimeout(() => {
                        m.delete();
                      }, 10000);
                      m.channel
                        ...
                        .then(m3 => {
                          ...
                          message.channel
                            ...
                            .then(m => {
                              m.edit(
                                message.member + ", **هل انت اداري بمتجر اخر ؟**"
                              );
                              setTimeout(() => {
                                m.delete();
                              }, 10000);
